import Canidate from "../Model/Candidate.Model.js";
// import cloudinary from "../config/cloudinary.js";
import uploadToCloudinary from "../utils/uploadToCloudinary.js";
import bcrypt from "bcryptjs";
import { sendVerificationEmail } from "../utils/mailer.js";
import Otp from "../Model/Candidate.Otp.Model.js";


// Delete Profile
export const deleteProfile = async (req, res) => {
  try {
    const user = await Canidate.findById(req.user.id);
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }

    await Canidate.findByIdAndDelete(req.user.id);
...
export const viewProfile = async (req, res) => {
  try {
    const user = await Canidate.findById(req.user.id).select("-password");
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const user = await Canidate.findById(userId);
    ...
    if (email && email !== user.email) {
      const existingUser = await Canidate.findOne({ email });
      if (existingUser) {
...
export const verifyUpdatedEmail = async (req, res) => {
  try {
    ...
    const user = await Canidate.findOne({ pendingEmail: email });
    if (!user) {
